...
use crate::dc::DcId;
use crate::pool::{ConnectionPool, ConnectionPurpose, PoolError};

// Re-export timeout and retry modules
pub use retry::{RetryError, RetryManager, RetryPolicy, RetryResult};
pub use timeout::{QueryTimeoutManager, TimeoutConfig, TimeoutError, TimeoutResult};
...
pub struct NetQueryDispatcher {
    ...
    /// Connection pool for acquiring sessions
    pool: parking_lot::Mutex<Option<Arc<ConnectionPool>>>,

    /// Session connection config for creating new connections
    session_config: parking_lot::Mutex<Option<crate::session::SessionConnectionConfig>>,
}
...
impl NetQueryDispatcher {
    ...
    pub fn new() -> Self {
        ...
        Self {
            ...
            session_config: parking_lot::Mutex::new(None),
        }
    }
    ...
    /// Sets the connection pool for acquiring sessions.
    pub fn set_session_pool(&self, pool: Arc<ConnectionPool>) {
        *self.pool.lock() = Some(pool);
    }

    /// Sets the session connection config for creating new connections.
    pub fn set_session_config(&self, config: crate::session::SessionConnectionConfig) {
        *self.session_config.lock() = Some(config);
    }
    ...
    pub fn dispatch(&self, query: NetQuery) -> Result<(), QueryError> {
        ...
        let pool = pool.ok_or_else(|| QueryError::Generic("No connection pool set".into()))?;

        // Get the session config
        let config = self.session_config.lock().clone();
        let config = config.unwrap_or_else(|| crate::session::SessionConnectionConfig::new(query.dc_id()));

        // Determine connection purpose from query type
        let purpose = match query.query_type() {
            NetQueryType::Download => ConnectionPurpose::Download,
            NetQueryType::Upload | NetQueryType::DownloadSmall => ConnectionPurpose::Upload,
            NetQueryType::Common => ConnectionPurpose::Main,
        };

        // Spawn a task to acquire connection and send query
        let dc_id = query.dc_id();
        let query_id = query.id();
        tokio::spawn(async move {
            // Acquire connection from pool
            match pool.acquire(dc_id, purpose, config).await {
                Ok(pooled) => {
                    let conn = pooled.connection();
                    tracing::trace!(
                        "Acquired connection for query {} to DC {:?}, purpose {:?}",
                        query_id,
                        dc_id,
                        purpose
                    );
                    // Send query through the session
                    if let Err(e) = conn.send_query(query) {
                        tracing::error!(
                            "Failed to send query {} to DC {:?}: {}",
                            query_id,
                            dc_id,
                            e
                        );
                ...
                Err(e) => {
                    ...
                    match &e {
                        PoolError::ConnectionFailed(msg) => {
                            if msg.contains("Socket error") || msg.contains("early eof") {
                                tracing::error!(
                                    "Connection failed for query {} to DC {:?}, purpose {:?}: {}. This usually indicates network issues or server unavailability.",
                                    query_id,
                                    dc_id,
                                    purpose,
                                    msg
                                ...
                                tracing::error!(
                                    "Connection failed for query {} to DC {:?}, purpose {:?}: {}",
                                    query_id,
                                    dc_id,
                                    purpose,
                                    msg
                        ...
                        PoolError::NoConnection { dc_id } => {
                            tracing::warn!(
                                "No connection available for query {} to DC {:?}, purpose {:?}. Connection pool may be exhausted.",
                                query_id,
                                dc_id,
                                purpose
                            );
                        ...
                        PoolError::Timeout { timeout } => {
                            tracing::warn!(
                                "Connection acquisition timeout for query {} to DC {:?}, purpose {:?} after {:?}",
                                query_id,
                                dc_id,
                                purpose,
                                timeout
                        ...
                        _ => {
                            tracing::error!(
                                "Failed to acquire connection for query {} to DC {:?}, purpose {:?}: {:?}",
                                query_id,
                                dc_id,
                                purpose,
                                e
